...
Chris.h is a header for C users, featuring rb-tree structure whose key can be any datatype the user defines, it might requires a few rules to use, which will be revealed once it's finished (some day), now have \_init_\_/insert/f_all/dfs for users
...
<code>Chris\*tree=(Chris\*)calloc(1,sizeof(Chris);
  _init_(tree,cmp,pnode);//cmp and pnodes are two functions defined by user
  //your operations
